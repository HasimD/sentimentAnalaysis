...
# NOT : SVM ile siniflandirma islemi cok uzun surebilir.
...
#---------------------- TFIDF ile TFIDF+EMOTICONS,SVM yontemlerini deneyelim ----------------------
...
y_pred = svc.predict(X_test) # olusturulan modele test verileri verilerek bir tahmin yapildi(TFIDF)

svc1 = SVC(kernel='linear',C=1)  
svc1.fit(X1_train, y1_train)
y1_pred = svc1.predict(X1_test) # olusturulan modele test verileri verilerek bir tahmin yapildi(TFIDF+Emoticons)
...
svc = SVC(kernel='linear',C=1)  
svc.fit(X_train, y_train)
y_pred = svc.predict(X_test) # olusturulan modele test verileri verilerek bir tahmin yapildi(cv)

svc1 = SVC(kernel='linear',C=1)  
svc1.fit(X1_train, y1_train)
y1_pred = svc1.predict(X1_test) # olusturulan modele test verileri verilerek bir tahmin yapildi(cv+Emoticons)
